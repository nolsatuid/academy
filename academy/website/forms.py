...
class CertificateVerifyForm(forms.Form):
    ...
    def verification(self, *args, **kwargs):
        ...
        graduates = Graduate.objects.filter(certificate_number__iexact=certificate_number).select_related('user').last()
        if graduates:
            if (graduates.user.last_name and graduates.user.last_name.lower() == last_name.lower()) or \
                (graduates.user.first_name and graduates.user.first_name.lower() == last_name.lower()):
                return graduates
            else:
                return None
